        ...
        for (int j = i == 0 ? 2 : 0; j < 32; j++) {
            ...
            if (m_ac3_output_block_index == 0) {
                ...
                if (m_last_burst_symbol_index != -1 && m_first_symbol_in_burst - m_last_burst_symbol_index > 3000 * 4) {
                    const size_t symbolGap = m_first_symbol_in_burst - m_last_burst_symbol_index;
                    m_log.warn(eAudio,
                        "Some input was not recognized as AC3 data and was discarded: " +
                        std::to_string(symbolGap) + " symbols / " +
                        std::to_string(symbolGap / 4) + " bytes / " +
                        std::to_string(symbolGap / (2880 * 4)) + " bursts");
                }
